...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = GaussianModel(dataset.sh_degree, opt.optimizer_type, pipe.isotropic_scaling)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        render_pkg = render(viewpoint_cam, gaussians, pipe, bg, use_trained_exp=dataset.train_test_exp, separate_sh=SPARSE_ADAM_AVAILABLE, is_train=True, iteration_ratio=iteration/opt.iterations)
        ...
        # Sensor Depth regularization        
        if depth_l1_weight(iteration) > 0 and viewpoint_cam.depth_reliable:            
            invDepth = render_pkg["depth"]
            sensor_invdepth = viewpoint_cam.sensorinvdepthmap.cuda()
            sensor_depth_mask = viewpoint_cam.sensor_depth_mask.cuda()

            Ll1sensordepth = torch.abs((invDepth  - sensor_invdepth) * sensor_depth_mask).sum() * (1.0/sensor_depth_mask.sum())
            loss += depth_l1_weight(iteration) * Ll1sensordepth 
            Ll1sensordepth = Ll1sensordepth.item()
        else:
            Ll1sensordepth = 0.0
        ...
        if depth_l1_weight(iteration) > 0  and viewpoint_cam.depth_reliable:
            invDepth = render_pkg["depth"]
            ml_invdepth = viewpoint_cam.mlinvdepthmap.cuda()
            ml_depth_mask = viewpoint_cam.ml_depth_mask.cuda()

            # Compute SIGNED gradients (preserve direction)
            depth_dx = (invDepth[:, :, 1:] - invDepth[:, :, :-1])
            depth_dy = (invDepth[:, 1:, :] - invDepth[:, :-1, :])         
            ml_dx = (ml_invdepth[:, :, 1:] - ml_invdepth[:, :, :-1]) 
            ml_dy = (ml_invdepth[:, 1:, :] - ml_invdepth[:, :-1, :])

            # Mask out invalid regions (where either pixel is invalid)
            mask_dx = (ml_depth_mask[:, :, 1:] * ml_depth_mask[:, :, :-1])
            mask_dy = (ml_depth_mask[:, 1:, :] * ml_depth_mask[:, :-1, :])

            Ll1mldepthgrad = (torch.abs((depth_dx - ml_dx) * mask_dx).sum() * (1.0/mask_dx.sum()) \
                            + torch.abs((depth_dy - ml_dy) * mask_dy).sum() * (1.0/mask_dy.sum()))
            loss += depth_l1_weight(iteration) * Ll1mldepthgrad
            Ll1mldepthgrad = Ll1mldepthgrad.item()
        else:
            Ll1mldepthgrad = 0
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:                            
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:                    
                    ...
                    print(f"current number of gaussians in the model: {gaussians._xyz.shape[0]}")
...
def training_report(tb_writer, iteration, Ll1, L1sensordepth, L1mldepthgrad, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, train_test_exp):
    ...
    if iteration in testing_iterations:
        ...
        validation_configs = ({'name': 'test', 'cameras' : scene.getTestCameras()}, 
                              {'name': 'train', 'cameras' : scene.getTrainCameras()}) #[scene.getTrainCameras()[idx % len(scene.getTrainCameras())] for idx in range(5, 30, 5)]})
            ...
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if viewpoint.depth_reliable:
                        invDepth = render_out["depth"]
                        gtInvDepth = viewpoint.sensorinvdepthmap.cuda()
                        gt_depth_mask = viewpoint.sensor_depth_mask.cuda()
                    # if train_test_exp:
                    ...
                    if tb_writer: # and (idx < 5):
                        ...
                        if viewpoint.depth_reliable:
                            tb_writer.add_images(config['name'] + "_view_{}/pred_invDepth".format(viewpoint.image_name), invDepth[None], global_step=iteration)                            
                        if iteration == testing_iterations[0]:
                            tb_writer.add_images(config['name'] + "_view_{}/ground_truth".format(viewpoint.image_name), gt_image[None], global_step=iteration)
                            if viewpoint.depth_reliable:                                
                                tb_writer.add_images(config['name'] + "_view_{}/gt_invDepth".format(viewpoint.image_name), gtInvDepth[None], global_step=iteration)
                    l1_test += l1_loss(image, gt_image).mean().double()
                    psnr_test += psnr(image, gt_image).mean().double()
                    if viewpoint.depth_reliable:
                        l1_depth_test += torch.abs((invDepth  - gtInvDepth) * gt_depth_mask).sum() * (1.0/gt_depth_mask.sum())
                psnr_test /= len(config['cameras'])
                l1_test /= len(config['cameras'])          
                if viewpoint.depth_reliable:
                    l1_depth_test /= len(config['cameras'])
                print("\n[ITER {}] Evaluating {}: L1 {} PSNR {}".format(iteration, config['name'], l1_test, psnr_test))
                if tb_writer:
                    tb_writer.add_scalar(config['name'] + '/loss_viewpoint - l1_loss', l1_test, iteration)
                    tb_writer.add_scalar(config['name'] + '/loss_viewpoint - psnr', psnr_test, iteration)                    
                    tb_writer.add_scalar(config['name'] + '/loss_viewpoint - l1_depth_loss', l1_depth_test, iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_iterations", nargs="+", type=int, default=list(range(10000, 30000, 1000))) # [7_000, 30_000])
