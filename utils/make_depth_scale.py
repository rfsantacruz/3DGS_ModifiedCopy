...
def get_scales(key, cameras, images, points3d_ordered, args):
    ...
    depth_file_path = f"{args.depths_dir}/{image_meta.name[:-n_remove]}.png"
    depth_file_path = depth_file_path.replace('colorimage', 'depthimage').replace('.png', '.tiff')
    invmonodepthmap = cv2.imread(depth_file_path, cv2.IMREAD_UNCHANGED) / 1000.0  # convert to meters
    invmonodepthmap[invmonodepthmap > 0] = 1.0 / invmonodepthmap[invmonodepthmap > 0]
